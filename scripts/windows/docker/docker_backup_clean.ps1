...
.DESCRIPTION
用于备份Docker镜像到指定目录，支持压缩和自动生成恢复脚本

.PARAMETER BackupDir
备份目录路径，默认为 D:\data\backup

.PARAMETER ImageNames
要备份的镜像名称列表，如果为空则备份所有镜像
...
.EXAMPLE
.\docker_backup.ps1 -BackupDir "C:\backup" -ImageNames @('nginx', 'redis')
备份指定镜像到指定目录

.EXAMPLE
.\docker_backup.ps1 -NoRun
仅进行语法检查
#>
